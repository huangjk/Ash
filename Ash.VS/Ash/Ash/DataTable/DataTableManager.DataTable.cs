...
namespace Ash.DataTable
{
    internal partial class DataTableManager
    {
        ...
        private sealed class DataTable<T> : DataTableBase, IDataTable<T> where T : class, IDataRow, new()
        {
            ...
            private EventHandler<AddDataRowEventArgs> m_AddDataRowEvent;
            private EventHandler<UpdateDataRowEventArgs> m_UpdateDataRowEvent;
            private EventHandler<DeleteDataRowEventArgs> m_DeleteDataRowEvent;
                ...
                : base(name)
            {
                ...
                m_AddDataRowEvent = null;
                m_UpdateDataRowEvent = null;
                m_DeleteDataRowEvent = null;
            }
            ...
            public event EventHandler<AddDataRowEventArgs> AddDataRowEvent
            {
                add
                {
                    m_AddDataRowEvent += value;
                }
            ...
            public event EventHandler<UpdateDataRowEventArgs> UpdateDataRowEvent
            {
                add
                {
                    m_UpdateDataRowEvent += value;
                }
            ...
            public event EventHandler<DeleteDataRowEventArgs> DeleteDataRowEvent
            {
                add
                {
                    m_DeleteDataRowEvent += value;
                }
            ...
            public void AddDataRow(T row)
            {
                ...
                if (m_AddDataRowEvent != null) m_AddDataRowEvent(this, new AddDataRowEventArgs(typeof(T), row, null));
            }
            ...
            public void DeleteDataRow(int id)
            {
                if (!HasDataRow(id))
                {
                    throw new AshException(string.Format("'{0}' isnt exist in data table '{1}'.", id.ToString(), Utility.Text.GetFullName<T>(Name)));
                }
                ...
                if (m_MinIdDataRow.Id == id)
                {
                    T newDataRow = null;
                    foreach (KeyValuePair<int, T> dataRowKVP in m_DataSet)
                    {
                        if (newDataRow == null) newDataRow = dataRowKVP.Value;
                        if (newDataRow.Id > dataRowKVP.Key) newDataRow = dataRowKVP.Value;
                    }
                    m_MinIdDataRow = newDataRow;
                }

                if (m_MaxIdDataRow.Id == id)
                {
                    T newDataRow = null;
                    foreach (KeyValuePair<int, T> dataRowKVP in m_DataSet)
                    {
                    ...
                    m_MaxIdDataRow = newDataRow;
                }

                if (m_DeleteDataRowEvent != null) m_DeleteDataRowEvent(this, new DeleteDataRowEventArgs(typeof(T), id, nT, null));
            }

            public void UpdateDataRow(T row)
            {
                T dataRow = row;
                if (!HasDataRow(dataRow.Id))
                {
                    throw new AshException(string.Format("'{0}' isnt exist in data table '{1}'.", dataRow.Id.ToString(), Utility.Text.GetFullName<T>(Name)));
                }

                m_DataSet[dataRow.Id] = dataRow;

                if(m_UpdateDataRowEvent!=null) m_UpdateDataRowEvent(this,new UpdateDataRowEventArgs(typeof(T), row.Id, row, null));
            }
        }
    }
}
