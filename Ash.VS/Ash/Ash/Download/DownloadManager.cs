...
namespace Ash.Download
{
    ...
    /// 下载管理器。
    /// </summary>
    internal sealed partial class DownloadManager : AshModule, IDownloadManager
    {
        ...
        public int AddDownload(string downloadPath, string downloadUri, object userData)
        {
            ...
            {
                throw new AshException("Download path is invalid.");
            }

            if (string.IsNullOrEmpty(downloadUri))
            {
                throw new AshException("Download uri is invalid.");
            }

            if (TotalAgentCount <= 0)
            {
                throw new AshException("You must add download agent first.");
            }
