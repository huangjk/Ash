...
namespace Ash.Download
{
    internal partial class DownloadManager
    {
        ...
        private sealed class DownloadAgent : ITaskAgent<DownloadTask>, IDisposable
        {
            ...
            public AshAction<DownloadAgent> DownloadAgentStart;
            public AshAction<DownloadAgent, int> DownloadAgentUpdate;
            public AshAction<DownloadAgent, int> DownloadAgentSuccess;
            public AshAction<DownloadAgent, string> DownloadAgentFailure;
            ...
            public void Start(DownloadTask task)
            {
                ...
                try
                {
                    ...
                    {
                        m_FileStream = System.IO.File.OpenWrite(downloadFile);
            ...
            private void OnDownloadAgentHelperComplete(object sender, DownloadAgentHelperCompleteEventArgs e)
            {
                ...
                {
                    throw new AshException("Internal download error.");
                }
                ...
                if (System.IO.File.Exists(m_Task.DownloadPath))
                {
                    System.IO.File.Delete(m_Task.DownloadPath);
                }

                System.IO.File.Move(string.Format("{0}.download", m_Task.DownloadPath), m_Task.DownloadPath);
