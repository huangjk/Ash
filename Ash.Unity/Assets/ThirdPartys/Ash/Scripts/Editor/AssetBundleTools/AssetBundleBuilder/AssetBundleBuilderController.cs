...
namespace AshUnityEditor.AssetBundleTools
{
    ...
    {
        private const string ConfigurationName = "ThirdPartys/Ash/Configs/AssetBundleBuilder.xml";
        ...
        public AssetBundleBuilderController()
        {
            ...
            m_BuildEventHandlerTypeNames.AddRange(AshUnityEditor.TypeHelper.GetEditorTypeNames(typeof(IBuildEventHandler)));
        ...
        public event AshAction<int, int> OnLoadingAssetBundle = null;

        public event AshAction<int, int> OnLoadingAsset = null;

        public event AshAction OnLoadCompleted = null;

        public event AshAction<int, int> OnAnalyzingAsset = null;

        public event AshAction OnAnalyzeCompleted = null;

        public event AshFunc<string, float, bool> ProcessingAssetBundle = null;

        public event AshAction<BuildTarget, string, int, int, int, int> ProcessAssetBundleComplete = null;

        public event AshAction<string> BuildAssetBundlesError = null;
        ...
        public bool Load()
        {
            ...
            try
            {
                ...
                XmlNode xmlRoot = xmlDocument.SelectSingleNode("Ash");
        ...
        public bool Save()
        {
            ...
            try
            {
                ...
                XmlElement xmlRoot = xmlDocument.CreateElement("Ash");
        ...
        {
            throw new AshException("Decompress is not implemented.");
        }
        ...
        private void BuildAssetBundles(AssetBundleBuild[] buildMap, BuildAssetBundleOptions buildOptions, bool zip, BuildTarget buildTarget)
        {
            ...
            if (Directory.Exists(workingPath))
            {
                ...
                {
                    if (!File.Exists(System.IO.Path.GetFileNameWithoutExtension(manifestName)))
                    {
        ...
        private void ProcessAssetBundle(string workingPath, string outputPackagePath, string outputFullPath, string outputPackedPath, bool zip, BuildTarget buildTarget, string assetBundleName, string assetBundleVariant)
        {
            ...
            string packageDirectoryName = System.IO.Path.GetDirectoryName(packageName);
            ...
            if (assetBundleData.Packed)
            {
                ...
                string packedDirectoryName = System.IO.Path.GetDirectoryName(packedName);
            ...
            string fullDirectoryName = System.IO.Path.GetDirectoryName(fullName);
        ...
        private void ProcessPackageList(string outputPackagePath, BuildTarget buildTarget)
        {
            ...
            using (FileStream fileStream = new FileStream(packageListPath, FileMode.CreateNew, FileAccess.Write))
            {
                using (BinaryWriter binaryWriter = new BinaryWriter(fileStream))
                {
                    ...
                    {
                        throw new AshException("Package list can only contains 65535 resources in version 0.");
                    }
                    ...
                    foreach (AssetBundleData assetBundleData in m_AssetBundleDatas.Values)
                    {
                        ...
                        {
                            throw new AshException(string.Format("AssetBundle name '{0}' is too long.", assetBundleData.Name));
                        }
                        ...
                        else
                        {
                            ...
                            {
                                throw new AshException(string.Format("AssetBundle variant '{0}' is too long.", assetBundleData.Variant));
                            }
                        ...
                        foreach (string assetName in assetNames)
                        {
                            ...
                            {
                                throw new AshException(string.Format("Asset name '{0}' is too long.", assetName));
                            }
                            ...
                            foreach (string dependencyAssetName in dependencyAssetNames)
                            {
                                ...
                                {
                                    throw new AshException(string.Format("Dependency asset name '{0}' is too long.", dependencyAssetName));
                                }
        ...
        private VersionListData ProcessVersionList(string outputFullPath, BuildTarget buildTarget)
        {
            ...
            using (FileStream fileStream = new FileStream(versionListPath, FileMode.CreateNew, FileAccess.Write))
            {
                using (BinaryWriter binaryWriter = new BinaryWriter(fileStream))
                {
                    ...
                    if (m_AssetBundleDatas.Count > ushort.MaxValue)
                    {
                        throw new AshException("Version list can only contains 65535 resources in version 0.");
                    }
