...
namespace AshUnity
{
    ...
    public sealed class UIForm : MonoBehaviour, IUIForm
    {
        ...
        public void OnInit(int serialId, string uiFormAssetName, IUIGroup uiGroup, bool pauseCoveredUIForm, bool isNewInstance, object userData)
        {
            ...
            Type uiLogicType = openUIFormInfo.UILogicType;

            if (uiLogicType == null)
            {
                Log.Error("UI logic type is invalid.");
                return;
            }

            if (m_UIFormLogic != null)
            {
                if (m_UIFormLogic.GetType() == uiLogicType)
                {
                    m_UIFormLogic.enabled = true;
                    return;
                }

                Destroy(m_UIFormLogic);
                m_UIFormLogic = null;
            }
            ...
            m_UIFormLogic = gameObject.GetOrAddComponent(uiLogicType) as UIFormLogic;
            if (m_UIFormLogic == null)
            {
                Log.Fatal("Can not get UIForm form logic. type: '{0}'" , uiLogicType);
                return;
            }

            m_UIFormLogic.OnInit(openUIFormInfo.UserData);
        }
        ...
        {
            OpenUIFormInfo openUIFormInfo = (OpenUIFormInfo)userData;
            m_UIFormLogic.OnOpen(openUIFormInfo.UserData);
        }
