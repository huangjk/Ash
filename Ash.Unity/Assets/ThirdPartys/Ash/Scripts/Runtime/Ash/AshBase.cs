...
namespace AshUnity
{
    ...
    public sealed partial class AshBase : AshComponent
    {
        ...
        [SerializeField]
        private RuningMode m_RuningMode = RuningMode.Auto;
        ...
        void Init()
        {
            ...
            {
                if (RuningMode == RuningMode.Auto) RuningMode = RuningMode.Develop;
            }
            else
            {
                if (RuningMode == RuningMode.Auto) RuningMode = RuningMode.Product;
            }
            Log.Info("RuningMode is " + RuningMode.ToString());

            m_EditorResourceMode &= Application.isEditor;
            if (m_EditorResourceMode)
            {
               
                Log.Info("During this run, Ash will use editor resource files, which you should validate first.");
            }
...
#else
            Log.Error("Ash only applies with Unity 5.3 and above, but current Unity version is {0}.", Application.unityVersion);
