...
namespace AppSettings
{
    ...
    public class SettingModule : SettingModuleBase
    {
        ...
        public static void WatchSetting(string path, System.Action<string> action)
        {
            if (!IsFileSystemMode)
            {
                Debug.LogError("[WatchSetting] Available in Unity Editor mode only!");
                return;
            }
            ...
            {
                Debug.LogFormat("Setting changed: {0}", e.FullPath);
                action(path);
            };
        }
#endif
