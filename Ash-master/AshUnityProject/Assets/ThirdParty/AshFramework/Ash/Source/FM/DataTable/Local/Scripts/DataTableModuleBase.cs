...
{
    /// <summary>
    /// For all class `XXXDataTables`
    /// </summary>
    public interface IReloadableDataTables
    {
        void ReloadAll();
        int Count { get; }
    }
    ...
    /// 使用时进行继承，并注册LoadDataTableMethod自定义的
    /// </summary>
    public abstract class DataTableModuleBase
    {
        /// <summary>
        /// table缓存
        /// </summary>
        ...
        /// </summary>
        protected abstract string LoadDataTable(string path);

        /// <summary>
        /// 通过DataTableModule拥有缓存与惰式加载
        /// </summary>
        ...
        public TableFile GetTableFile(string path, bool useCache = false) 
        {
            ...
            {
                var fileContent = LoadDataTable(path);
