...
namespace Ash.Core.WebRequest
{
    ...
    /// Web 请求管理器。
    /// </summary>
    internal sealed partial class WebRequestManager : AshModule, IWebRequestManager
    {
        ...
        public int AddWebRequest(string webRequestUri, byte[] postData, object userData)
        {
            ...
            {
                throw new AshException("Web request uri is invalid.");
            }

            if (TotalAgentCount <= 0)
            {
                throw new AshException("You must add web request agent first.");
            }
