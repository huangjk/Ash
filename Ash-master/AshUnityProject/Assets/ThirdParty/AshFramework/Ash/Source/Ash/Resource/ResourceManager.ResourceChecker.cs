...
namespace Ash.Core.Resource
{
    internal partial class ResourceManager
    {
        ...
        private sealed partial class ResourceChecker
        {
            ...
            public AshAction<ResourceName, LoadType, int, int, int, int> ResourceNeedUpdate;
            public AshAction<int, int, int, int> ResourceCheckComplete;
            ...
            private void RefreshCheckInfoStatus()
            {
                ...
                foreach (KeyValuePair<ResourceName, CheckInfo> checkInfo in m_CheckInfos)
                {
                    ...
                    {
                        throw new AshException(string.Format("Check resources '{0}' error with unknown status.", ci.ResourceName.FullName));
                    }
                    ...
                    if (ci.NeedRemove)
                    {
                        ...
                        {
                            throw new AshException(string.Format("Resource '{0}' is not exist in read-write list.", ci.ResourceName.FullName));
                        }
            ...
            private void ParseVersionList(string fileUri, byte[] bytes, string errorMessage)
            {
                ...
                {
                    throw new AshException("Version list has been parsed.");
                }

                if (bytes == null || bytes.Length <= 0)
                {
                    throw new AshException(string.Format("Version list '{0}' is invalid, error message is '{1}'.", fileUri, string.IsNullOrEmpty(errorMessage) ? "<Empty>" : errorMessage));
                }
                ...
                try
                {
                    ...
                    using (BinaryReader binaryReader = new BinaryReader(memoryStream))
                    {
                        ...
                        {
                            throw new AshException("Version list header is invalid.");
                        }
                        ...
                        if (listVersion == 0)
                        {
                            ...
                            for (int i = 0; i < resourceGroupCount; i++)
                            {
                                ...
                                for (int j = 0; j < groupResourceCount; j++)
                                {
                                    ...
                                    {
                                        throw new AshException(string.Format("Version index '{0}' is invalid, resource count is '{1}'.", versionIndex, resourceCount));
                                    }
                        ...
                        {
                            throw new AshException("Version list version is invalid.");
                        }
                    }
                ...
                catch (Exception exception)
                {
                    ...
                    throw new AshException(string.Format("Parse version list exception '{0}'.", exception.Message), exception);
                }
                finally
                {
            ...
            private void ParseReadOnlyList(string fileUri, byte[] bytes, string errorMessage)
            {
                ...
                {
                    throw new AshException("Readonly list has been parsed.");
                }
                ...
                try
                {
                    ...
                    using (BinaryReader binaryReader = new BinaryReader(memoryStream))
                    {
                        ...
                        {
                            throw new AshException("Readonly list header is invalid.");
                        }
                        ...
                        {
                            throw new AshException("Readonly list version is invalid.");
                        }
                    }
                ...
                catch (Exception exception)
                {
                    ...
                    throw new AshException(string.Format("Parse readonly list exception '{0}'.", exception.Message), exception);
                }
                finally
                {
                    if (memoryStream != null)
            ...
            private void ParseReadWriteList(string fileUri, byte[] bytes, string errorMessage)
            {
                ...
                {
                    throw new AshException("Read-write list has been parsed.");
                }
                ...
                try
                {
                    ...
                    using (BinaryReader binaryReader = new BinaryReader(memoryStream))
                    {
                        ...
                        {
                            throw new AshException("Read-write list header is invalid.");
                        }
                        ...
                        if (listVersion == 0)
                        {
                            ...
                            for (int i = 0; i < resourceCount; i++)
                            {
                                ...
                                {
                                    throw new AshException(string.Format("Read-write resource info '{0}' is already exist.", resourceName.FullName));
                                }
                        ...
                        {
                            throw new AshException("Read-write list version is invalid.");
                        }
                    }
                ...
                catch (Exception exception)
                {
                    ...
                    throw new AshException(string.Format("Parse read-write list exception '{0}'.", exception.Message), exception);
                }
                finally
                {
                    if (memoryStream != null)
